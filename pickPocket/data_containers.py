...
#Class variable
    ...
    ############
    # interesting for other properties also, like confidence..
...
# on a very small subset of clusters which are recognised as pockets. So to save data I preferred to separate them.
    ...
    # def traceTag(self):
    #     """
    #     Returns a list of probe coordinates at tag=1,2 and 3 with associated flag (for plotting purposes)
    #     """
    ...
    def pArea_matchScore(self,ligand_coordinates,scoreTh=0.2):
        ...
        TODO:distinguish between different ligands, give results separately..
        """
        coord = self.getCoordinates()
    ...
    def matchScore(self,ligand_coordinates,distanceTh = 5, matchScoreTh=0.5, coverageTh=0.2):
        ...
        if((np.round(matchScore,1)>=matchScoreTh)and(np.round(coverageScore,1)>=coverageTh)):
            success = True
            # print(success)
            # print("\n MATCH SCORE = %.1f%% \t COVERAGE SCORE = %.1f%%" %(np.round(matchScore,3)*100,np.round(coverageScore,3)*100))
        else:
            success = False
            # print(success)
            #PROVVISORIO DEBUG
            # if(matchScore>=0.5):
            #     print("Coverage score failed")
            #     print("\n MATCH SCORE = %.1f%% \t COVERAGE SCORE = %.1f%%" %(np.round(matchScore,3)*100,np.round(coverageScore,3)*100))
            # else:
            #     print("\n MATCH SCORE = %.1f%%" %(np.round(matchScore,3)*100))
            # matchScore = 0
            # coverageScore = 0
