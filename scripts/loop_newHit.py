...
def main():
    ...
    if(isTest):
        ...
        hitTop3OnlySub =np.zeros((alphas.size,betas.size,radii.size,n_models))
        hitTopOnlySub1 = np.zeros((alphas.size,betas.size,radii.size,n_models))
        hitTopOnlySub2 = np.zeros((alphas.size,betas.size,radii.size,n_models))
        hitTopOnlySubFar = np.zeros((alphas.size,betas.size,radii.size,n_models))
        hitTopFarSub = np.zeros((alphas.size,betas.size,radii.size,n_models))

        hitTop1Sub = np.zeros((alphas.size,betas.size,radii.size,n_models))
        hitTop2Sub = np.zeros((alphas.size,betas.size,radii.size,n_models))
        ...
        top3SingleHit= np.zeros((alphas.size,betas.size,radii.size,n_models))
        largePHitTop3 = np.zeros((alphas.size,betas.size,radii.size,n_models))
        largePHitTop10 = np.zeros((alphas.size,betas.size,radii.size,n_models))
        ...
        nSubs=np.zeros((alphas.size,betas.size,radii.size,n_models))
        ...
        for s in range(n_structures):
            ...
            for ligand_name in ligands:
                ...
                if((ligand_name[1] == True) and excludePeptide):
                    ...
                    continue
                try:
                    coord = readLigands(global_module.pdbFolder_path+ligand_name[0])
                    if (len(coord)==0):
            ...
            for i,alpha in enumerate(alphas):
                for j,beta in enumerate(betas):
                    for k,rp_max in enumerate(radii):
                        ...
                        rankedIndexesIF,rankedIndexesSubIF,numericalScoreIF,numericalScoreSubIF = scoreIF.getRankingOnlyChem(featListChem)
                        rankIF_onlyChem = (rankedIndexesIF,rankedIndexesSubIF)
                        score_onlyChem = (numericalScoreIF,numericalScoreSubIF)

                        rankedIndexesIF,rankedIndexesSubIF,numericalScoreIF,numericalScoreSubIF = scoreIF.getRankingOnlyGeom(featListGeom)
                        ...
                        for ln,l in enumerate(ligands_coord):
                            ...
                            for m in range(n_models):
                                ...
                                while ((r<10) and (r_in < rankMain.size)):
                                    ...
                                    if(si is not None):
                                        ...
                                        gotHit= pocket.CMmatchScore(l['coord'])
                                        if gotHit:
                                            # print("single pocket hit")
                                            # print("rank: %d, internal index: %d"%(r+1,r_in))
                                        ...
                                        if gotHit:
                                            ...
                                            if(saveP and m==0):
                                                ...
                                                saveResSimple(r,save_path,pocket,clustering.get_protein().resMap)
                                            if(n_subs <2):
                                                # print("small pocket hit")
                                                # print("rank: %d, internal index: %d"%(r+1,r_in))
                                                singleHit[i,j,k,m] +=1 #counter of how many times pocket with 0 or 1 subpockets hit
                                                if(r<3):
                                                    top3SingleHit[i,j,k,m] +=1
                                            # else:
                                            #     print("large pocket hit")
                                            #     print("rank: %d, internal index: %d"%(r+1,r_in))
                                        ...
                                        for ns,sub_i in enumerate(indS):
                                            ...
                                            gotHitsub = subpocket.CMmatchScore(l['coord'])
                                            #print(ns)
                                            ...
                                            if (gotHitsub): 
                                                ...
                                                # print("rank: %d, internal index: %d"%(r+1,r_in))
                                                # print('model:', names[m])
                                                # print('here: p%d sub%d %d'%(r,ns,sub_i))
                                                #ONLY TOP 3 SUBS MATTER FOR PROMOTION OF MASTER POCKET
                                                subscore = scoreSub[internal_rank[sub_i]]
                                                

                                                if(n_subs>1):
                                                    avScoreSubs[i,j,k,m] += subscore
                                                    if(ns < 3):
                                                        hitTopWithSub[i,j,k,m] += 1 # counter indicating if the one hitting also hit within top3 pocket (if more than 1)
                                                        if ns==0:
                                                            hitTop1Sub[i,j,k,m] += 1 # counter indicating if the one hitting also hit with top1 subpocket (if more than 1)
                                                        elif ns==1:
                                                            hitTop2Sub[i,j,k,m] += 1
                                                    else:
                                                        hitTopFarSub[i,j,k,m] += 1 
                                                    if(not gotHit):
                                                        ...
                                                        if(r<3):
                                                            hitTop3OnlySub[i,j,k,m] +=1
                                                        if (ns<3):
                                                            if ns==0:
                                                                hitTopOnlySub1[i,j,k,m]+=1
                                                            elif ns==1:
                                                                hitTopOnlySub2[i,j,k,m]+=1
                                                        else:
                                                            hitTopOnlySubFar[i,j,k,m] += 1
                                                ...
                                                if(not gotHit):
                                                    ...
                                                    if(r<3):
                                                        ...
                                                        avScoreTop3[i,j,k,m] += score
                                                    if(n_subs==1):
                                                        singleHit[i,j,k,m] +=1 #Single subpocket will be promoted to master
                                                        if(r<3):
                                                            top3SingleHit[i,j,k,m] +=1
                                                #ATTENTION: NOW DO NOT OVERWRITE..
                                                # OS_kept[m] = OS
                                                # VS_kept[m] = VS
                                        ...
                                        if((not gotHitsub)and gotHit and (n_subs>1)):
                                            if(r<3):
                                                largePHitTop3[i,j,k,m] +=1
                                            else:
                                                largePHitTop10[i,j,k,m] +=1
                                        if gotHit:
                                            #OF MASTER POCKET
                                            hitNsubs[m] = n_subs
                                            break
                            ...
                            # Reomving initial break on master pocket, I can now evaluate if both the pocket and the subpockets did hit
                            # or only the subpocket: this counter represent if only the subpocket hit
                            # That's why I removed bitwise or here:
                            # top10withSub = np.bitwise_or(top10withSub,top10)
                            # top3withSub = np.bitwise_or(top3withSub,top3)

                            # # DEBUG
                            # print("top10withSub",top10withSub)
                            # print("top10withoutSub",top10)
                            # print("top3withSub",top3withSub)
                            # print("top3withoutSub",top3)
                                #within model loop
            ...
            avHitTop1Sub = np.nan_to_num(hitTop1Sub/normForsub)
            avHitTop2Sub = np.nan_to_num(hitTop2Sub/normForsub)

            avlargePTop3 = np.nan_to_num(largePHitTop3/norm) #was normForsub--> difficult to assess contribution within top10
            avlargePTop10 = np.nan_to_num(largePHitTop10/norm)

            avSingleHit = singleHit/norm
            avTop3SingleHit =  np.nan_to_num(top3SingleHit/singleHit) #among single hits, which where top3 classified?
            avHitTopOnlySub = np.nan_to_num(hitTopOnlySub/norm)
            avHitTop3OnlySub= np.nan_to_num(hitTop3OnlySub/norm)
            avHitTopOnlySub1 = np.nan_to_num(hitTopOnlySub1/norm)
            avHitTopOnlySub2 = np.nan_to_num(hitTopOnlySub2/norm)
            avHitTopOnlySubFar = np.nan_to_num(hitTopOnlySubFar/norm)

            avHitTopFarSub = np.nan_to_num(hitTopFarSub/normForsub)
            ...
            average_nSubs = np.nan_to_num(nSubs/allHit)

            #Averaged scores

            avScore_log=np.nan_to_num(avScore/hitTop10)
            avScoreTop3_log = np.nan_to_num(avScoreTop3/hitTop3)
            avScoreSubs_log = np.nan_to_num(avScoreSubs/hitTopWithSub)
