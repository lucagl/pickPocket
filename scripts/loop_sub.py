...
def main():
    ...
    if(isTest):
        ...
        names = ["IF_1topsubs.out","IF_2topsubs.out","IF_3topsubs.out","IF_allTopsubs.out"]
        ...
        ff = [open("noTopHit_1topsubs_"+modelName+".out",'w'),open("noTopHit_2topsubs_"+modelName+".out",'w'),open("noTopHit_3topsubs_"+modelName+".out",'w'),
        open("noTopHit_alltopsubs_"+modelName+".out",'w')]
        ...
        for s in range(n_structures):
                    ...
                    for k,rp_max in enumerate(radii):
                        ...
                        for ln,l in enumerate(ligands_coord):
                            ...
                            for m in range(n_models):
                                ...
                                subThreshold = subTh[m]

                                print("\nmodel subThreshold = ",subThreshold)
                                    ...
                                    if(si is not None):
                                        ...
                                        for ns,sub_i in enumerate(indS):
                                            ...
                                            if (gotHitsub): 
                                                ...
                                                hitMatrix[i,j,k,m,:] += hitCounter(r)
                                                hitTop10[i,j,k,m] +=1
                                                avScore[i,j,k,m] += score #SCORE OF MASTER..
                                                if(r<3):
                                                    hitTop3[i,j,k,m] += 1
                                                    avScoreTop3[i,j,k,m] += score                               
                                                OS_kept[m] = OS
            ...
            # SAVING METHOD.. 
            for m in range(n_models):
                save_rankingStats_simple(m,alphas,betas,radii,avHitTop3,avHitTop10,
                avSingleHit,average_OS_log,average_VS_log,average_volume_log,sum_nPockets/analStructures,average_nSubs,
